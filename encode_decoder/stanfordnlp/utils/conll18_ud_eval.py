...
ID, LEMMA, FORM, UPOS, XPOS, FEATS, HEAD, DEPREL, DEPS, MISC = range(10)
...
def load_conllu(file):
    ...
    while True:
        ...
        array = line.split('\t')
        # if len(array) < 4 or not (
        #         "sg" in array[5] # array[3] == "S" and
                # array[5] == "sg p" or array[5] == "sg g" or array[5] == "sg n"  # array[3] == "S" and
                # array[5] == "_"):  # array[3] == "S" and
                # ):
            # continue
        # if line.startswith("#"):
        #     continue
...
def evaluate(gold_ud, system_ud):
    class Score:
        def __init__(self, gold_total, system_total, correct, aligned_total=None):
            # print('f1')
            # print(2 * correct / (system_total + gold_total) if system_total + gold_total else 0.0)
        ...
        for words in alignment.matched_words:
            if filter_fn is None or filter_fn(words.gold_word):
                if key_fn(words.gold_word, gold_aligned_gold) == key_fn(words.system_word, gold_aligned_system):
                    correct += 1
                elif isinstance(key_fn(words.system_word, gold_aligned_system), str) and \
                    words.system_word.columns[LEMMA] == key_fn(words.system_word, gold_aligned_system) and \
                    key_fn(words.gold_word, gold_aligned_gold).lower() in [x.lower() for x in key_fn(words.system_word, gold_aligned_system).split("||")]:
                    correct += 1
