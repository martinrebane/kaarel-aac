...
def parse_args():
    ...
    parser.add_argument('--continue_train', type=bool, default=False, help='If true, training will continue from model')
...
def train(args):
    ...
    if args["continue_train"]:
        print("[Training continued from previous model...]")
        trainer = Trainer(use_cuda=args['cuda'], model_file=model_file)
        if isinstance(trainer.args["dev_score"], list):
            dev_score_history = trainer.args["dev_score"]
        elif isinstance(trainer.args["dev_score"], float):
            dev_score_history += [trainer.args["dev_score"]]
    ...
    trainer.train_dict(train_batch.conll.get(['word', 'upos', 'lemma', 'feats']))
    print("Evaluating on dev set...")
    dev_preds = trainer.predict_dict(dev_batch.conll.get(['word', 'upos', 'feats']))
    ...
    if args.get('dict_only', False):
        ...
        for epoch in range(1, args['num_epoch']+1):
            ...
            if args.get('ensemble_dict', False):
                ...
                dev_preds = trainer.ensemble(dev_batch.conll.get(['word', 'upos', 'feats']), dev_preds)
            dev_batch.conll.write_conll_with_lemmas(dev_preds, system_pred_file)
            ...
            print(dev_score_history)
            if "dev_score" in trainer.args:
                if isinstance(trainer.args["dev_score"], list):
                    trainer.args["dev_score"] += [dev_score]
                elif isinstance(trainer.args["dev_score"], float):
                    trainer.args["dev_score"] = [dev_score]
            else:
                trainer.args["dev_score"] = [dev_score]
...
def evaluate(args):
    ...
    dict_preds = trainer.predict_dict(batch.conll.get(['word', 'upos', 'feats']))
    ...
    if args['dict_only']:
        ...
        if args['ensemble_dict']:
            print("[Ensembling dict with seq2seq lemmatizer...]")
            preds = trainer.ensemble(batch.conll.get(['word', 'upos', 'feats']), preds)
