...
class Seq2SeqModel(nn.Module):
    ...
    def forward(self, src, src_mask, tgt_in, pos=None):
        ...
        if self.use_pos:
            ...
            # dec_inputs = torch.cat([dec_inputs, pos_inputs.unsqueeze(1)], dim=1)
            # enc_inputs = torch.cat([enc_inputs, pos_inputs.unsqueeze(1)], dim=1)
            # enc_inputs = torch.cat([enc_inputs, pos_inputs.unsqueeze(1)], dim=1)
            # enc_inputs = torch.cat([enc_inputs, pos_inputs.unsqueeze(1)], dim=1)
            # enc_inputs = torch.cat([enc_inputs, pos_inputs.unsqueeze(1)], dim=1)
    ...
    def predict(self, src, src_mask, pos=None, beam_size=5):
        ...
        if beam_size <= 1:
            ...
            all_hyp, all_scores, sec_best_hyp, thi_best_hyp, for_best_hyp, fiv_best_hyp = [], [], [], [], [], []
            all_hyps = []
            for b in range(batch_size):
                scores, ks = beam[b].sort_best()
                all_hyp = []
                for i, score in enumerate(scores):
                    ...
                    hyp = utils.prune_hyp(hyp)
                    all_hyp += [hyp]
                all_hyps.append(all_hyp)

            # print('all_hyp')
            # print(all_hyp)
            # print(all_scores)
            # print(sec_best_hyp)
            return all_hyps, edit_logits
