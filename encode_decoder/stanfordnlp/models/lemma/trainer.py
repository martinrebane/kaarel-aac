...
class Trainer(object):
    ...
    def update(self, batch, eval=False):
        ...
        # tgt_out = torch.cat([tgt_out, pos.unsqueeze(1)], dim=1)
    ...
    def predict(self, batch, beam_size=1):
        ...
        if beam_size <= 1:
            ...
            tokens = []
            for pred in preds:
                pred_seqs = [self.vocab['char'].unmap(ids) for ids in pred] # unmap to tokens
                pred_seqs = utils.prune_decoded_seqs(pred_seqs)
                pred_tokens = ["".join(seq) for seq in pred_seqs] # join chars to be tokens
                tokens.append('||'.join([str(x) for x in pred_tokens]))
    ...
    def train_dict(self, triples):
        ...
        # accumulate counter
        ctr = Counter()
        ctr.update([(p[0], p[3], p[2]) for p in triples])
        # ctr.update([(p[0], p[1] + '|' + p[3], p[2]) for p in triples])
    ...
    def predict_dict(self, pairs):
        ...
        for p in pairs:
            w, pos, feats = p
            if (w,feats) in self.composite_dict:
                lemmas += [self.composite_dict[(w, feats)]]
        ...
        #     print(lemmas[i] + ": " + '/ '.join([str(x) for x in w]))
        return lemmas
